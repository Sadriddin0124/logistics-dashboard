...
export const fetchFinanceInfo = async (start_date: string, end_date: string) => {
    const response = await $api.get(`/finance/info?start_date=${start_date}&end_date=${end_date}`);
    return response.data;
  };
