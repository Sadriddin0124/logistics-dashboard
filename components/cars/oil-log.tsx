...
export function OilLog() {
  ...
  const { data: purchased } = useQuery<PaginatedRouteLog>({
    queryKey: ["car-oil", currentPage, id],
    queryFn: () => fetchAutoOil(id as string, currentPage),
    enabled: !!id,
  });
  useEffect(() => {
    queryClient.prefetchQuery({
      queryKey: ["car-oil", id, currentPage + 1],
      queryFn: () => fetchAutoOil(id as string, currentPage + 1),
    });
  }, [currentPage, id]);
      ...
      <Table>
        <TableHeader className="font-bold">
          <TableRow className="border-b-gray-300 border-b">
            <TableHead className="font-bold">Масла</TableHead>
