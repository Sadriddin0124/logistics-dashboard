...
import { BarChart2, ClipboardList, Menu, Package, Settings, Users } from "lucide-react";
...
export const AppSidebar: React.FC<SideBarProps> = ({setSideBar, setSubItemStatus, subItemStatus}) => {
  ...
  return (
    <aside className="fixed top-0 left-0 h-full z-50">
      <Sidebar
        className={`${subItemStatus ? "w-[600px]" : "w-[300px]"} transition-none`}
      ...
      >
        ...
        >
          ...
          <div className="flex w-full mb-3 justify-start py-3 px-6 items-center gap-[100px]">
            ...
            <div onClick={()=>setSideBar(prev=> !prev)} className="relative">
            ...
            {menuItems?.map((item, index) => {
              return (
                ...
                >
                  {pathname.split("/")[1] === item?.key && <div className="rounded-r-xl absolute w-2 h-full bg-[#4880FF] left-0"></div>}
                  <Link
                    href={item?.href || ""}
                    className={`${pathname.split("/")[1] === item?.key ? " bg-[#4880FF] text-white" : "text-black hover:bg-slate-100 "} py-3 flex items-end gap-2 rounded-[8px] pl-3 w-[80%]`}
          ...
          {subItemStatus && (
            <div className="bg-white rounded-xl box_shadow p-8 overflow-hidden w-full h-[80%] mt-[30%] flex flex-col gap-1">
              {activeSubItem?.map((item, index) => {
                return (
                  <Link href={item.href || ""} key={index} className={`${pathname === item?.href ? "text-[#4880FF] font-bold bg-[#487fff36]" : ""} hover:text-[#4880FF] hover:font-bold rounded-[4px] text-[14px] hover:bg-[#487fff36] py-3 px-4 text`} onClick={() => setSubItemStatus(false)}>
                    {item?.title}
                  </Link>
                );
              })}
